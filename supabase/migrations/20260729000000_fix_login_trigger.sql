...
AS $function$
 ...
 BEGIN
   ...
   INSERT INTO public.user_profiles (id, email, role, login, cpf, cnpj, registration_type, organization_id, sponsor_id, created_at, updated_at)
   VALUES (new.id, new.email, COALESCE(new.raw_user_meta_data ->> 'role', 'affiliate'), new.raw_user_meta_data ->> 'login', new.raw_user_meta_data ->> 'cpf', new.raw_user_meta_data ->> 'cnpj', new.raw_user_meta_data ->> 'registration_type', v_org_id, v_sponsor_id, new.created_at, new.created_at);
