      ...
      <ul className={`space-y-1 ${level > 0 ? 'ml-4 mt-1 border-l border-slate-100 pl-2' : ''}`}>
        {nodes.map(node => {
          ...
          return (
            <li key={node.id}>
              <div
                className={`group flex items-center justify-between py-2 px-3 rounded-xl cursor-pointer transition-all duration-200 ${isActive ? 'bg-[#2980B9] text-[#0B1221] font-bold shadow-md shadow-[#2980B9]/20' : 'hover:bg-slate-50 text-slate-600'
                  }`}
                onClick={() => {
    ...
    <div className="space-y-6">
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-[#0B1221] flex items-center gap-2">
          <Filter className="w-5 h-5 text-[#2980B9]" />
          Categorias
        </h3>
        <div className="h-1 w-12 bg-[#2980B9] rounded-full"></div>
      </div>

      <div className="space-y-2">
        <button
          onClick={() => onCategorySelect(null)}
          className={`text-sm w-full text-left py-2 px-2 rounded-lg transition-colors ${activeCategoryId === null ? 'bg-[#2980B9]/10 text-[#0B1221] font-bold' : 'text-slate-600 hover:bg-slate-50'
            }`}
        >
          Todos os Produtos
        </button>
